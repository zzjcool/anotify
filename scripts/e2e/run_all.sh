...
echo "▶ 构建二进制…"
mkdir -p .e2e-bin
go build -o .e2e-bin/anotify ./cmd/server || { echo "❌ server 构建失败"; exit 1; }
go build -o .e2e-bin/devseed ./cmd/devseed || { echo "❌ devseed 构建失败"; exit 1; }
...
if go test ./... -count=1 > /tmp/e2e-unit.log 2>&1; then
  ...
  echo "  ❌ go test 失败（见 /tmp/e2e-unit.log）"; tail -15 /tmp/e2e-unit.log; UNIT_FAIL=1
fi

# 3. 逐套件运行
SUITES=(api_contract auth_flow ws_protocol routing persistence security edge_cases frontend push_e2e)
ONLY="${1:-}"
