...
async function main() {
	...
	try {
		...
		const devResp = await H.req(server.base, "/v1/devices", { session, body: dev });
		...
		const beforeList = await H.req(server.base, "/v1/notifications?limit=50", { session });
		H.eq("重启前消息数=3", beforeList.json?.count, 3);
		const maxSeqBefore = Math.max(...beforeList.json.notifications.map((m) => m.Seq));
		...
		// 2. 消息仍在
		const afterList = await H.req(server.base, "/v1/notifications?limit=50", { session });
		H.eq("重启后消息仍在（count=3）", afterList.json?.count, 3);
		const titles = afterList.json.notifications.map((m) => m.Title).sort();
		H.eq("重启后消息内容一致", titles, ["持久化消息1", "持久化消息2", "持久化消息3"]);
		...
		const finalList = await H.req(server.base, "/v1/notifications?limit=50", { session });
		const newMsg = finalList.json.notifications.find((m) => m.Title === "重启后上报");
		H.check("重启后新消息存在", !!newMsg);
		H.eq("seq 连续（接续不重置）", newMsg?.Seq, maxSeqBefore + 1);

		// 6. 会话仍有效
		H.eq("重启后会话仍有效", (await H.req(server.base, "/v1/notifications", { session })).status, 200);
