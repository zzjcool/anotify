...
async function createDevice(name, endpointTag) {
	...
	if (r.status !== 200) throw new Error(`创建设备 ${name} 失败: ${r.status} ${r.text}`);
	return r.json.device;
}

// 用 PATCH 设置设备配置
async function patchDevice(id, patch) {
	const r = await H.req(server.base, `/v1/devices/${id}`, { session, method: "PATCH", body: patch });
	if (r.status !== 200) throw new Error(`PATCH 设备 ${id} 失败: ${r.status} ${r.text}`);
	return r.json.device;
}
...
async function main() {
	...
	await patchDevice(a.id, { tags: ["手机"], statusFilter: "all", enabled: true });
	dev.A = a.id;

	// B: tags=["工作"], filter=error, enabled
	const b = await createDevice("B-工作", "b");
	await patchDevice(b.id, { tags: ["工作"], statusFilter: "error", enabled: true });
	dev.B = b.id;
	...
	await patchDevice(d.id, { tags: ["手机"], statusFilter: "all", enabled: false });
	dev.D = d.id;
	...
	H.eq("A 配置生效(tags=[手机],filter=all,enabled)", [byId[dev.A]?.tags?.join(), byId[dev.A]?.statusFilter, byId[dev.A]?.enabled], ["手机", "all", true]);
	H.eq("B 配置生效(tags=[工作],filter=error,enabled)", [byId[dev.B]?.tags?.join(), byId[dev.B]?.statusFilter, byId[dev.B]?.enabled], ["工作", "error", true]);
	H.eq("C 配置生效(tags=[],filter=all,enabled)", [(byId[dev.C]?.tags || []).length, byId[dev.C]?.statusFilter, byId[dev.C]?.enabled], [0, "all", true]);
	H.eq("D 配置生效(enabled=false)", byId[dev.D]?.enabled, false);

	// ---- 路由断言矩阵 ----
	// 1. success 无 deviceTags → 广播；B 的 filter=error 过滤 success；D 禁用 → 命中 A,C
	expectHits("case1 success 广播", await notify({ title: "c1", status: "success" }), [dev.A, dev.C]);

	// 2. error 无 deviceTags → 广播；B 的 error 通过；D 禁用 → 命中 A,B,C
	expectHits("case2 error 广播", await notify({ title: "c2", status: "error" }), [dev.A, dev.B, dev.C]);

	// 3. error + deviceTags=["手机"] → A(tag+status 过), C(catch-all)；B tag 不符 → 命中 A,C
	expectHits("case3 error 定向[手机]", await notify({ title: "c3", status: "error", deviceTags: ["手机"] }), [dev.A, dev.C]);

	// 4. error + deviceTags=["工作"] → B(tag+status 过), C(catch-all)；A 不符 → 命中 B,C
	expectHits("case4 error 定向[工作]", await notify({ title: "c4", status: "error", deviceTags: ["工作"] }), [dev.B, dev.C]);

	// 5. success + deviceTags=["手机","工作"] → A(tag过+all过), C；B 被 status 过滤 → 命中 A,C
	expectHits("case5 success 定向[手机,工作]", await notify({ title: "c5", status: "success", deviceTags: ["手机", "工作"] }), [dev.A, dev.C]);

	// 6. interrupted 无 deviceTags → A,C；B=error filter 不过 interrupted → 命中 A,C
	expectHits("case6 interrupted 广播", await notify({ title: "c6", status: "interrupted" }), [dev.A, dev.C]);

	// 7. success + deviceTags=["不存在"] → 仅 C(catch-all) → matched=1
	expectHits("case7 success 定向[不存在]", await notify({ title: "c7", status: "success", deviceTags: ["不存在"] }), [dev.C]);

	// 8. 专项：禁用设备 D 在所有定向到其 tag 的情况下也不接收
	{
		const r = await notify({ title: "c8", status: "success", deviceTags: ["手机"] });
		H.check("case8 禁用设备 D 永不命中（定向其 tag）", !r.hitIds.has(dev.D));
	}
	// 8b. info/warning 广播 → 仅 all 通过（A,C），B=error 不过
	expectHits("case8b info 广播", await notify({ title: "c8b", status: "info" }), [dev.A, dev.C]);
	expectHits("case8c warning 广播", await notify({ title: "c8c", status: "warning" }), [dev.A, dev.C]);
